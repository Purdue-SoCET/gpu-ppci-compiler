...
from ..binutils.layout import Layout, Memory, Section, SymbolDefinition, EntrySymbol
...
parser.add_argument("-c", action="store_true", default=False, help="Compile only, do not link")
parser.add_argument("-ld", "--layout", help="Custom layout file (overrides default MMIO layout)", metavar="LAYOUT")
parser.add_argument("--entry", default="main", help="Entry symbol for linking (default: main)")
parser.add_argument("--hex-output", default="meminit.hex", help="Output file for custom 32-bit binary strings")
...
def twig(args=None):
    ...
    with LogSetup(args) as log_setup:
        ...
        if args.E:
            ...
            ir_modules = []
            for src in args.sources:
                ir_module = api.c_to_ir(src, march, coptions=coptions, reporter=log_setup.reporter)
            ...
            if args.ir or args.c or args.S:
                ...
                output_filename = args.output if args.output else args.name+".out"
