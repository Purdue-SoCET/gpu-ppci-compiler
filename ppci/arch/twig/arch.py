...
class TwigArch(Architecture):
    ...
    def immUsed(self, r1, r2, offset, instruction, pred=0):
        ...
        self.logger.debug(
            "immUsed large offset: offset=%d (0x%08X), upper=0x%02X, middle=0x%03X, lower=0x%03X, instr=%s",
            offset,
            offset_u32,
            upper_8,
            middle_12,
    ...
    def peephole(self, frame):
        ...
        for ins in frame.instructions:
            if hasattr(ins, "fprel") and ins.fprel:
                ...
                self.logger.debug(
                    "peephole fprel: ins=%s raw_offset=%s final_offset=%s stacksize=%s callee_save_space=%s pred=%s",
                    ins,
                ...
                if final_offset < -(1 << 31) or final_offset > 0x7FFFFFFF:
                    raise ValueError(
                        f"fprel final_offset out of 32-bit signed range: {final_offset}"
                    )
