...
def get_register_classes(rfc_size=0):
    """
    Returns the register classes for the allocator.
    Excludes the top `rfc_size` registers from the pool so they can be
    reserved exclusively for the Register File Cache (RFC) optimization.
    """
    # The full list of standard allocatable registers (ending at R62)
    all_allocatable_regs = [
        R9, R10, R11, R12, R13, R14, R15, R16, R17, R18, R19, R20, R21,
        R22, R23, R24, R25, R26, R27, R32, R33, R34, R35, R36, R37, R38,
        R39, R40, R41, R42, R43, R44, R45, R46, R47, R48, R49, R50, R51,
        R52, R53, R54, R55, R56, R57, R58, R59, R60, R61, R62
    ]

    # Strip the RFC registers from the standard allocator pool
    standard_regs = all_allocatable_regs[:-rfc_size] if rfc_size > 0 else all_allocatable_regs
