...
def write_packet_histogram_svg(
    ...
    lines = [
        ...
        (
            ...
            f'font-family="sans-serif" font-size="{title_font}">'
    ...
    for tick in range(5):
        ...
        lines.append(
            ...
            f'font-family="sans-serif" font-size="{tick_font}">'
            f"{count_value}</text>"
        )
    ...
    for packet_size in range(1, max_packet_size + 1):
        ...
        lines.append(
            ...
            f'y="{margin_top + chart_height + 16}" '
            f'text-anchor="middle" font-family="sans-serif" '
            f'font-size="{tick_font}">{packet_size}</text>'
        )

        if count:
            lines.append(
                f'<text x="{x + bar_width / 2}" '
                f'y="{max(y - 4, margin_top - 2)}" '
                f'text-anchor="middle" font-family="sans-serif" '
                f'font-size="{value_font}">{count}</text>'
            )

    lines.extend(
        [
            (
                f'<text x="{width / 2}" y="{height - 14}" '
                f'text-anchor="middle" font-family="sans-serif" '
                f'font-size="{axis_font}">Packet size</text>'
            ),
            (
