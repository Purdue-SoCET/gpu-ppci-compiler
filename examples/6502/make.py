    ...
    with open(os.path.join("c64disk", "hello.prg"), "wb") as f:
        ...
        basic_program = bytes(
            [
                ...
                0x08,  # Load address
                0x09,
                0x08,  # start of the next line
                0x0A,
                0x00,  # line number 10 word in little endianness
                # 0x9e, 0x20, 0x34, 0x30, 0x39, 0x36, 0x00,  # SYS 4096
                0x99,  # Print token!
                0x20,
                0x31,  # Space and number 1
                0x0,  # End of line
                0x1A,
                0x00,  # 0x1a line number in little endianness
                0x99,  # Print token!
                0x20,
                0x22,  # Space and "
            ]
        ...
        for nr, line in enumerate(text_message.split("\n")):
            ...
            program.append(
                BasicLine(
                    30 + nr, bytes([0x99, 0x20, 0x22]) + line + bytes([0x22])
                )
            )
        sys_address = 0x890
