 ...
 */
void kernel_stack_verify(void* arg) {
    stack_verify_arg_t* args = (stack_verify_arg_t*)arg;
    ...
    args->results[base + 4] = (read_a == expected_a && read_b == expected_b &&
                               read_c == expected_c && read_d == expected_d) ? 1u : 0u;
}
