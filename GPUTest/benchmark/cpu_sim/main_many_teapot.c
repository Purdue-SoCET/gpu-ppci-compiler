...
int main(int argc, char** argv) {
    ...
    loadbin("cpu_sim/data/geometry/teapot1K.bin", &teapot2);
    ...
    const int num_verts = teapot.vertsN + teapot2.vertsN;
    const int num_tris = teapot.trisN + teapot2.trisN;
    ...
    for (int i = 0; i < num_verts; i++) {
        if(i < teapot.vertsN) {
            ...
            verts[i] = teapot2.vertices[i-teapot.vertsN]; 
        }
    }
    ...
    for (int i = 0; i < num_tris; i++) {
        if(i < teapot.trisN) {
            ...
            tris[i].v1 = teapot2.triangles[i-teapot.trisN].v1 + teapot.vertsN; 
            tris[i].v2 = teapot2.triangles[i-teapot.trisN].v2 + teapot.vertsN; 
            tris[i].v3 = teapot2.triangles[i-teapot.trisN].v3 + teapot.vertsN; 
        }
    }
    ...
    free(teapot2.vertices);
    free(teapot2.triangles);
